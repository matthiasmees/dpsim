...
cmake \
	...
	-DOpenMP_CXX_FLAGS="-Xpreprocessor -fopenmp -I${OPENMP_INCLUDE_DIR}" \
...
PIP_CMAKE_ARGS="-DCMAKE_PREFIX_PATH=${CMAKE_PREFIX_PATH_VALUE} -DEigen3_DIR=${EIGEN3_DIR} -DGraphviz_ROOT=${GRAPHVIZ_PREFIX} -DOpenMP_ROOT=${OPENMP_PREFIX} -DOpenMP_CXX_FLAGS=-Xpreprocessor\\ -fopenmp\\ -I${OPENMP_INCLUDE_DIR} -DOpenMP_CXX_LIB_NAMES=omp -DOpenMP_omp_LIBRARY=${OPENMP_LIBRARY} -DWITH_OPENMP=ON"
...
Reinstall the Python package after binding/package changes with:
  ...
  CMAKE_ARGS="-DCMAKE_PREFIX_PATH=${CMAKE_PREFIX_PATH_VALUE} -DEigen3_DIR=${EIGEN3_DIR} -DGraphviz_ROOT=${GRAPHVIZ_PREFIX} -DOpenMP_ROOT=${OPENMP_PREFIX} -DOpenMP_CXX_FLAGS=-Xpreprocessor\\ -fopenmp\\ -I${OPENMP_INCLUDE_DIR} -DOpenMP_CXX_LIB_NAMES=omp -DOpenMP_omp_LIBRARY=${OPENMP_LIBRARY} -DWITH_OPENMP=ON" \\
  DPSIM_PYTHON_WITH_VILLAS=0 \\
  python -m pip install . -v
