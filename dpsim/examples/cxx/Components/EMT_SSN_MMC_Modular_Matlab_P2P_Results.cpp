...
void configureMmcCommon(const EMT::Ph3::MMC_Modular::Ptr &mmc,
                        ...
                        UInt linearizationUpdateInterval) {
  const Real vBaseAc = std::sqrt(2.0 / 3.0) * p.secondaryVoltage;
  const Real iBaseAc = (2.0 / 3.0) * p.ratedPower / vBaseAc;
  ...
  // Correctness-first modular policy. Interval 1 reproduces the original
  // variable-SSN behavior; larger values enable affine-model reuse only after
  // the interval-1 regression agrees with the established SSN_MMC result.
  mmc->setStructuredLinearization(true);
  mmc->setLinearizationUpdateInterval(linearizationUpdateInterval);
  mmc->setRuntimeDiagnostics(false);
  mmc->setTheta(theta);
}
...
int main(int argc, char **argv) {
  ...
  const UInt linearizationUpdateInterval =
      argc > 5 ? static_cast<UInt>(std::stoul(argv[5])) : 1;
  ...
  require(linearizationUpdateInterval > 0,
          "The linearization interval must be at least one EMT step.");

  const String simName = "EMT_SSN_MMC_Modular_Matlab_P2P_Results";
  ...
  SPDLOG_INFO("Modular MATLAB P2P regression: dt={} s, finalTime={} s, "
              "theta={}, logEvery={} steps ({} s), linearizeEvery={} "
              "steps ({} s), Pref_initial={} W, Vdc=[{},{}] V, Idc={} A",
              timeStep, finalTime, theta, logEveryNthStep,
              logEveryNthStep * timeStep, linearizationUpdateInterval,
              linearizationUpdateInterval * timeStep, op.inverterPower.real(),
  ...
  auto rectifier = EMT::Ph3::MMC_Modular::make("MMC_MODULAR_RECTIFIER");
  auto inverter = EMT::Ph3::MMC_Modular::make("MMC_MODULAR_INVERTER");
  ...
  for (const auto &[prefix, mmc] :
       std::vector<std::pair<String, EMT::Ph3::MMC_Modular::Ptr>>{
  ...
  sim.setSystemMatrixRecomputationMode(
  ...
  SPDLOG_INFO("Simulation completed at t={} s. Linearization counts: "
              "REC[rebuild={},reuse={}], INV[rebuild={},reuse={}]. "
              "Plot data are available under logs/{}/.",
              sim.time(), rectifier->linearizationRebuildCount(),
              rectifier->linearizationReuseCount(),
              inverter->linearizationRebuildCount(),
              inverter->linearizationReuseCount(), simName);
