...
struct Parameters {
  ...
  // The current Scenario A case starts the GFL at zero P/Q.
  Real gflInitialActivePower = 0.0;
  Real gflInitialReactivePower = 0.0;
  Real gflReactivePowerAfterStep = -15e6;

  // GFL inverse-droop gains
  Real gflFrequencyToActivePowerGainPu = 4.0; // * 0.0;
  Real gflVoltageToReactivePowerGainPu = 2.0; // * 0.0;
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) {
    ...
    if (args.options.find("PV_Q_STEP_TIME") != args.options.end())
      p.gflReactivePowerStepTime = args.getOptionReal("PV_Q_STEP_TIME");

    if (args.options.find("PV_Q_AFTER_STEP") != args.options.end())
      p.gflReactivePowerAfterStep = args.getOptionReal("PV_Q_AFTER_STEP");
  ...
  if (!(p.frequency > 0.0) || !(p.timeStep > 0.0) || !(p.finalTime > 0.0) ||
      !(p.logDownSampling > 0) || !(p.gflReactivePowerStepTime >= 0.0) ||
    ...
    throw std::invalid_argument(
        ...
        "0<=gflReactivePowerStepTime<finalTime and "
  ...
  SPDLOG_INFO("Scenario A bases:"
              ...
              p.pshRatedVoltage, p.pshInertia, p.gflRatedPower,
              ...
              p.trPshResistance, p.trPshInductance, p.breakerOpenResistance,
              p.breakerClosedResistance, p.gflFilterResistancePu(),
  ...
  // 1. POWER FLOW
  //
  // PF and EMT use the same node/component names and the same electrical graph:
  //
  ...
  //                             GEN_psh
  //
  // The breaker is OPEN during PF and EMT initialization.
  //
  // GEN_gas and GEN_psh are real SP::Ph1::SynchronGenerator components.
  // Because the breaker is open, each machine is the VD source of its island.
  //
  // The GFL is represented in PF by a zero-power SP::Ph1::Load named "pv".
  // This matches the current Scenario A operating point and gives BUS_pv a
  // solved voltage. Since the GFL starts at exactly P=Q=0, the generic
  // load-power transfer is also numerically/sign-convention neutral here.
  // ==========================================================================
  ...
  // PF TR_pv: same orientation as EMT, network side -> converter side.
  auto trPvPF =
  ...
  // PF surrogate for the GFL. Current Scenario A initialization is zero P/Q.
  ...
  SystemTopology systemPF(p.frequency,
                          SystemNodeList{busGasPF, busBPF, busAPF, busPshaPF,
                                         busTrPshPF, busPshPF, busPvPF},
                          SystemComponentList{gasGeneratorPF, trGasPF, cablePF,
                                              line3PF, breakerPF, trPshPF,
                                              pshGeneratorPF, trPvPF, pvPF});
  ...
  SPDLOG_INFO("PF operating point:"
              ...
              "\n  GFL_pv : P={} MW, Q={} Mvar, |V|={} V RMS, angle={} deg"
              ...
              pshGeneratorPF->getApparentPower().real() / 1e6,
              pshGeneratorPF->getApparentPower().imag() / 1e6,
  ...
  SystemTopology systemEMT(
      ...
      SystemComponentList{gasGenerator, trGas, cable, line3, breaker, trPsh,
                          pshGenerator, trPv, pvGfl});

  // Central PF -> EMT initialization.
  //
  // The branch version of SystemTopology::initWithPowerflow() transfers:
  //   - solved node voltages by exact node name,
  //   - synchronous-generator terminal P/Q by exact component name,
  //   - SP load terminal P/Q by exact component name.
  //
  // Therefore no generatorPositivePower(), terminal()->setPower() calls, or
  // manually copied initial phasors are needed in this example.
  ...
  const Real qAfterStepPu = p.gflReactivePowerAfterStep / p.gflRatedPower;

  simulationEMT.addEvent(AttributeEvent<Real>::make(
      qStepTime, pvGfl->mReactivePowerRefPu, qAfterStepPu));

  // Close the three-phase breaker and connect the PSH island.
  simulationEMT.addEvent(
      SwitchEvent3Ph::make(p.breakerCloseTime, breaker, true));

  SPDLOG_INFO("\nStarting Scenario A GFL EMT simulation:"
              "\n  dt={} s"
              "\n  finalTime={} s"
              "\n  GFL Q step: {} Mvar at t={} s"
              "\n  breaker closes at t={} s"
              "\n  initial breaker state=open"
              "\n  matrix recomputation={}",
              p.timeStep, p.finalTime, p.gflReactivePowerAfterStep / 1e6,
              qStepTime, p.breakerCloseTime);
