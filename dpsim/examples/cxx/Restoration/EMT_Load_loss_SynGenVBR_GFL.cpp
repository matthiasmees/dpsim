...
struct Parameters {
  ...
  Real trPvRatioMagnitude = 11.0e3 / 220e3;
...
int main() {
  ...
  // Pv transformer
  auto pvTransformer = SP::Ph1::Transformer::make("TR_pv", "TR_pv",
                                                  Logger::Level::debug, false);
  pvTransformer->setParameters(
      p.trPvVoltageLow, p.trPvVoltageHigh, p.trPvRatedPower,
      p.trPvRatioMagnitude, 0.0,
      p.trPvResistance, p.trPvInductance);
  pvTransformer->setBaseVoltage(p.trPvVoltageHigh);

  // Pv
  auto pvEquivalent = SP::Ph1::Load::make("pv", Logger::Level::debug);
  pvEquivalent->setParameters(0,
                              0,
                              11e3);
  ...
  pvTransformer->connect({busPvPF, busBPF});
  pvEquivalent->connect({busPvPF});

  SystemTopology systemPF(
      p.frequency, SystemNodeList{busGasPF, busBPF, busLoadPF, busLoad2bPF, busPvPF},
      SystemComponentList{gasGeneratorPF, trGasPF, trLoadPF, load2aPF,
                          breakerPF, load2bPF, pvTransformer, pvEquivalent});
  ...
  pvTransformer->setParameters(
      p.trPvVoltageLow, p.trPvVoltageHigh, p.trPvRatedPower,
      p.trPvVoltageLow / p.trPvVoltageHigh, 0.0,
  ...
  pvTransformer->connect({busPv, busB});
  pvGfl->connect({busPv});

  SystemTopology systemEMT(p.frequency,
                           SystemNodeList{busGas, busB, busLoad, busLoad2b, busPv},
                           SystemComponentList{gasGenerator, trGas, trLoad,
                                               load2a, breaker, load2b, pvTransformer, pvGfl});
