...
///
/// DPsim's interface-current vector y is positive from each MNA node into the
/// component. The internal differential current iDelta follows the MATLAB
/// converter convention and is positive from the converter into the AC grid.
/// Therefore y_ac = -abc(iDelta).
///
/// Positive iSigma_z is DC current absorbed by the converter:
///   i_dc,abs = 3*iSigma_z,
///   y_dc+ = +i_dc,abs, y_dc- = -i_dc,abs.
/// With the MATLAB/Harmony AC convention, positive iDelta_d and positive
/// active power denote AC power exported by the converter.
///
/// Active and reactive power attributes are generation-positive, matching the
/// MATLAB example:
///   P = 3/2*(v_d*iDelta_d + v_q*iDelta_q),
///   Q = 3/2*(v_q*iDelta_d - v_d*iDelta_q).
///
/// The Park transform uses q = -sin(theta). The MMC plant depends on
/// v_dc = v_dc+ - v_dc- and is invariant to a common-mode shift of the two DC
/// pole voltages.
///
...
public:
  ...
  /// Active power is positive when generated by the converter.
  void setActivePowerControl(Real activePowerReference, Real kp, Real ki);

  /// MATLAB Controllers3 feedforward path:
  ///
  ///   iDelta_d_ref = (2/3) * P_ref / Vd_filtered
  ///
  /// Vd_filtered is produced by a critically damped second-order low-pass
  /// filter. The filter and current reference are updated at controlTimeStep
  /// and held between samples. This is a sampled digital-control path and is
  /// intentionally kept outside the continuous SSN state vector.
  void setActivePowerFeedforwardControl(Real activePowerReference,
                                        Real voltageFilterCutoffFrequency,
                                        Real controlTimeStep,
                                        Real minimumDaxisVoltage = 0.0);

  /// Change P_ref without resetting the sampled voltage-filter states. The
  /// reference is applied at the next controller sample.
  void setActivePowerFeedforwardReference(Real activePowerReference);

  /// MATLAB VDC regulator convention: e_vdc = vdc_measured - vdc_reference.
  ...
  /// iSigmaZReference > 0 commands DC current absorption by the converter
  /// (normally corresponding to positive AC power export).
  ...
  /// Differential-voltage reference selected by the plant before modulation
  /// normalization, as a 2x1 [d,q] vector in peak phase volts.
  ...
  Attribute<Matrix>::Ptr realizedConverterVoltageAttribute() const;
  ...
  /// Diagnostic-only local continuous linearization. The surrounding
  /// AC/DC MNA network is deliberately not included.
...
protected:
  ...
  void initializeSampledActivePowerFeedforward(const Matrix &x0,
                                               const Matrix &u0);
  void advanceSampledActivePowerFeedforward();
  ...
  // Sampled MATLAB Controllers3 P_ref/Vd path.
  Real mActivePowerFeedforwardFilterCutoffFrequency;
  Real mActivePowerFeedforwardControlTimeStep;
  Real mActivePowerFeedforwardMinimumDaxisVoltage;
  Real mActivePowerFeedforwardFilteredVd;
  Real mActivePowerFeedforwardFilteredVdDerivative;
  Real mActivePowerFeedforwardHeldIDeltaDReference;
  UInt mActivePowerFeedforwardStepCounter;
  Bool mActivePowerFeedforwardInitialized;
  Bool mActivePowerFeedforwardRuntimeEnabled;
  ...
  Attribute<Matrix>::Ptr mAppliedDifferentialVoltage;
  Attribute<Matrix>::Ptr mInternalDifferentialVoltage;
  Attribute<Matrix>::Ptr mAppliedCommonModeVoltage;
  Attribute<Matrix>::Ptr mAppliedModulation;
  Attribute<Matrix>::Ptr mRealizedConverterVoltage;
